...
with open("IRC.txt", "r") as f:
	...
	def write_ct(fd, line):
	    fd.write('  , ("{0}", {1})\n'.format(line[0], line[1].title().replace("_", "")))

	def write_ic(fd, line):
	    fd.write('                | {}\n'.format(line[1].title().replace("_", "")))
	...
	with open("src/IrcCommand.hs", "w") as ic:
	    ...
	    ic.write("\n")
	    ic.write("data IrcCommand = {command}\n".format(command=line[1].title().replace("_", "")))

	    ct.write('    ("{string}", {command})\n'.format(
